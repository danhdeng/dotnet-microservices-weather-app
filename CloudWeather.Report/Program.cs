...
builder.Services.AddDbContext<WeatherReportDbContext>(
    options =>
    {
        ...
        options.UseNpgsql(builder.Configuration.GetConnectionString("WeatherReportDb"));
    ...
    async (string zip, [FromQuery] int? days, IWeatherReportAggregator weatherReportAggregator) =>
{
    ...
    {
        return Results.BadRequest("Please enter a 'days' query paramter or days must be greater than 1 and less than 30");
    }
    var reports=await weatherReportAggregator.BuildReport(zip, days);
